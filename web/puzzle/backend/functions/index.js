...
const { payout } = require('./contract/call_shards');
...
const shardNum = 4;
const randShardID = () => {
  return Math.floor(Math.random() * shardNum);
};
  ...
  try {
    ...
    if (doc.empty) {
      ...
      if (doc_address != address) {
        ...
        payout(randShardID(), address, level, sequence)
          .then(async r => {
